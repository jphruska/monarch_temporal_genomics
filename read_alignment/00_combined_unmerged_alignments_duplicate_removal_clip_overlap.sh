#!/bin/bash
#SBATCH --chdir=./
#SBATCH --job-name=combined_unmerged_alignments_duplicate_removal_clip_overlap
#SBATCH --partition batch
#SBATCH --nodes=1 --ntasks=12
#SBATCH --time=48:00:00
#SBATCH --mem-per-cpu=8G
#SBATCH --array=1-180
#SBATCH -o /work/sonsthagen/johruska/monarchs/scripts/read_alignment/logs/combined_unmerged_dup_removal_clip/%j.out
...
-M ${workdir}/00_bam_files/${basename_array}_unmerged_combined_markdups_metric_file.txt \
-I ${workdir}/00_bam_files/${basename_array}_unmerged_combined.bam \
-O ${workdir}/00_bam_files/${basename_array}_unmerged_combined_dup_rm.bam
...
--in ${workdir}/00_bam_files/${basename_array}_unmerged_combined_dup_rm.bam \
--out ${workdir}/00_bam_files/${basename_array}_unmerged_combined_dup_rm_clip.bam \
--unmapped \
--stats
...
I=${workdir}/00_bam_files/${basename_array}_unmerged_combined_dup_rm_clip.bam \
